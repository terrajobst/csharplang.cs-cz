...
<span data-ttu-id="5d87d-122">Pole deklarované bez `static` Modifikátor je volána ***proměnnou instance***.</span><span class="sxs-lookup"><span data-stu-id="5d87d-122">A field declared without the `static` modifier is called an ***instance variable***.</span></span>
...
*  <span data-ttu-id="5d87d-162">Každou výstupní parametr funkce člena nebo anonymní funkce musí být jednoznačně přiřazena ([jednoznačného přiřazení](variables.md#definite-assignment)) před funkce člena nebo anonymní funkce vrátí normálně.</span><span class="sxs-lookup"><span data-stu-id="5d87d-162">Every output parameter of a function member or anonymous function must be definitely assigned ([Definite assignment](variables.md#definite-assignment)) before the function member or anonymous function returns normally.</span></span>
...
<span data-ttu-id="5d87d-181">Následující kategorie proměnné jsou automaticky inicializovány na výchozí hodnoty:</span><span class="sxs-lookup"><span data-stu-id="5d87d-181">The following categories of variables are automatically initialized to their default values:</span></span>
...
*  <span data-ttu-id="5d87d-210">Proměnná musí být jednoznačně přiřazena v každém umístění, kde je předán jako parametr odkazu.</span><span class="sxs-lookup"><span data-stu-id="5d87d-210">A variable must be definitely assigned at each location where it is passed as a reference parameter.</span></span> <span data-ttu-id="5d87d-211">Tím se zajistí, že můžete členské funkce volané zvažte referenční parametr původně přiřazený.</span><span class="sxs-lookup"><span data-stu-id="5d87d-211">This ensures that the function member being invoked can consider the reference parameter initially assigned.</span></span>
...
<span data-ttu-id="5d87d-225">Následující kategorie proměnné jsou klasifikovány jako původně nepřiřazené:</span><span class="sxs-lookup"><span data-stu-id="5d87d-225">The following categories of variables are classified as initially unassigned:</span></span>
...
*  <span data-ttu-id="5d87d-239">Jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-239">Definitely assigned.</span></span> <span data-ttu-id="5d87d-240">To znamená, že na všechny toky možné ovládací prvek do této chvíle *v* byla přiřazena hodnota.</span><span class="sxs-lookup"><span data-stu-id="5d87d-240">This indicates that on all possible control flows to this point, *v* has been assigned a value.</span></span>
...
#### <a name="switch-statements"></a><span data-ttu-id="5d87d-275">Příkazy Switch</span><span class="sxs-lookup"><span data-stu-id="5d87d-275">Switch statements</span></span>
...
<span data-ttu-id="5d87d-286">Pro `do` příkaz *příkazu Insert* formuláře:</span><span class="sxs-lookup"><span data-stu-id="5d87d-286">For a `do` statement *stmt* of the form:</span></span>
```csharp
...
<span data-ttu-id="5d87d-296">Stav jednoznačného přiřazení *v* na přenos řízení toku způsobené `break`, `continue`, nebo `goto` příkaz je stejné jako stav jednoznačného přiřazení *v* na začátek příkazu.</span><span class="sxs-lookup"><span data-stu-id="5d87d-296">The definite assignment state of *v* on the control flow transfer caused by a `break`, `continue`, or `goto` statement is the same as the definite assignment state of *v* at the beginning of the statement.</span></span>
...
#### <a name="try-catch-statements"></a><span data-ttu-id="5d87d-310">Try-catch – příkazy</span><span class="sxs-lookup"><span data-stu-id="5d87d-310">Try-catch statements</span></span>
...
<span data-ttu-id="5d87d-337">Pro `lock` příkaz *příkazu Insert* formuláře:</span><span class="sxs-lookup"><span data-stu-id="5d87d-337">For a `lock` statement *stmt* of the form:</span></span>
```csharp
...
*  <span data-ttu-id="5d87d-347">Stav jednoznačného přiřazení *v* na konci takový výraz je stejné jako stav jednoznačného přiřazení *v* na začátku výrazu.</span><span class="sxs-lookup"><span data-stu-id="5d87d-347">The definite assignment state of *v* at the end of such an expression is the same as the definite assignment state of *v* at the beginning of the expression.</span></span>
...
*  <span data-ttu-id="5d87d-354">Stav jednoznačného přiřazení *v* na začátku *e1* je stejný jako jednoznačného přiřazení stavu na začátku *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-354">The definite assignment state of *v* at the beginning of *e1* is the same as the definite assignment state at the beginning of *expr*.</span></span>
...
*  <span data-ttu-id="5d87d-371">Stav jednoznačného přiřazení *v* po *expr* se určuje podle:</span><span class="sxs-lookup"><span data-stu-id="5d87d-371">The definite assignment state of *v* after *expr* is determined by:</span></span>
   ...
   * <span data-ttu-id="5d87d-373">Jinak, pokud dojde k přiřazení v rámci konstruktoru instance typu struktury, pokud *w* je přístup k vlastnosti s vyznačením automaticky implementovanou vlastnost *P* vytváří instance a *v* je pole Skrytá zálohování *P*, pak jednoznačného přiřazení stavu *v* po *expr* se o výborný prostředek přiřadit.</span><span class="sxs-lookup"><span data-stu-id="5d87d-373">Otherwise, if the assignment occurs within the instance constructor of a struct type, if *w* is a property access designating an automatically implemented property *P* on the instance being constructed and *v* is the hidden backing field of *P*, then the definite assignment state of *v* after *expr* is definitely assigned.</span></span>
   * <span data-ttu-id="5d87d-374">V opačném případě jednoznačného přiřazení stavu *v* po *expr* je stejné jako stav jednoznačného přiřazení *v* po *expr_rhs*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-374">Otherwise, the definite assignment state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_rhs*.</span></span>

#### <a name="-conditional-and-expressions"></a><span data-ttu-id="5d87d-375">& & (podmiňovací operátor AND) výrazů</span><span class="sxs-lookup"><span data-stu-id="5d87d-375">&& (conditional AND) expressions</span></span>

<span data-ttu-id="5d87d-376">Pro výraz *expr* formuláře `expr_first && expr_second`:</span><span class="sxs-lookup"><span data-stu-id="5d87d-376">For an expression *expr* of the form `expr_first && expr_second`:</span></span>

*  <span data-ttu-id="5d87d-377">Stav jednoznačného přiřazení *v* před *expr_first* je stejné jako stav jednoznačného přiřazení *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-377">The definite assignment state of *v* before *expr_first* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="5d87d-378">Stav jednoznačného přiřazení *v* před *expr_second* je jednoznačně přiřazovat, pokud státu *v* po *expr_first* je buď určitě přiřazené nebo "přiřazené jednoznačně po Výraz true".</span><span class="sxs-lookup"><span data-stu-id="5d87d-378">The definite assignment state of *v* before *expr_second* is definitely assigned if the state of *v* after *expr_first* is either definitely assigned or "definitely assigned after true expression".</span></span> <span data-ttu-id="5d87d-379">V opačném případě ji není jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-379">Otherwise, it is not definitely assigned.</span></span>
*  <span data-ttu-id="5d87d-380">Stav jednoznačného přiřazení *v* po *expr* se určuje podle:</span><span class="sxs-lookup"><span data-stu-id="5d87d-380">The definite assignment state of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="5d87d-381">Pokud *expr_first* je konstantní výraz s hodnotou `false`, pak jednoznačného přiřazení stavu *v* po *expr* je stejný jako jednoznačného přiřazení Stav *v* po *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-381">If *expr_first* is a constant expression with the value `false`, then the definite assignment state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_first*.</span></span>
    * <span data-ttu-id="5d87d-382">Jinak, pokud státu *v* po *expr_first* jednoznačně přiřazena, potom stav *v* po *expr* jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-382">Otherwise, if the state of *v* after *expr_first* is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-383">Jinak, pokud stav *v* po *expr_second* jednoznačně přiřazena a stav *v* po *expr_first* je "jednoznačně přiřazené po false výraz"a pak stav *v* po *expr* jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-383">Otherwise, if the state of *v* after *expr_second* is definitely assigned, and the state of *v* after *expr_first* is "definitely assigned after false expression", then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-384">Jinak, pokud státu *v* po *expr_second* jednoznačně přiřazena nebo "přiřazené jednoznačně po hodnotu true, výraz", potom stav *v* po  *výraz* "jednoznačně přiřazena po Výraz true".</span><span class="sxs-lookup"><span data-stu-id="5d87d-384">Otherwise, if the state of *v* after *expr_second* is definitely assigned or "definitely assigned after true expression", then the state of *v* after *expr* is "definitely assigned after true expression".</span></span>
    * <span data-ttu-id="5d87d-385">Jinak, pokud státu *v* po *expr_first* je "přiřazené jednoznačně po false výraz" a stav *v* po *expr_second* "přiřazené jednoznačně po false výraz", je stav *v* po *expr* "jednoznačně přiřazena po výrazu false".</span><span class="sxs-lookup"><span data-stu-id="5d87d-385">Otherwise, if the state of *v* after *expr_first* is "definitely assigned after false expression", and the state of *v* after *expr_second* is "definitely assigned after false expression", then the state of *v* after *expr* is "definitely assigned after false expression".</span></span>
    * <span data-ttu-id="5d87d-386">V opačném případě stav *v* po *expr* není jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-386">Otherwise, the state of *v* after *expr* is not definitely assigned.</span></span>

<span data-ttu-id="5d87d-387">V příkladu</span><span class="sxs-lookup"><span data-stu-id="5d87d-387">In the example</span></span>
```csharp
class A
{
...
```
<span data-ttu-id="5d87d-388">proměnné `i` se považuje za jednoznačně přiřazené v jednom z vložené příkazy `if` příkazu, ale ne v jiném.</span><span class="sxs-lookup"><span data-stu-id="5d87d-388">the variable `i` is considered definitely assigned in one of the embedded statements of an `if` statement but not in the other.</span></span> <span data-ttu-id="5d87d-389">V `if` příkaz v metodě `F`, proměnná `i` je jednoznačně přiřazena první příkaz vložený, protože provádění výrazu `(i = y)` vždy předchází spuštění tohoto příkazu embedded.</span><span class="sxs-lookup"><span data-stu-id="5d87d-389">In the `if` statement in method `F`, the variable `i` is definitely assigned in the first embedded statement because execution of the expression `(i = y)` always precedes execution of this embedded statement.</span></span> <span data-ttu-id="5d87d-390">Naproti tomu, proměnná `i` není jednoznačně přiřazena v druhém příkazu vložený, protože `x >= 0` může otestovali false, což vede k proměnné `i` se nepřiřazený.</span><span class="sxs-lookup"><span data-stu-id="5d87d-390">In contrast, the variable `i` is not definitely assigned in the second embedded statement, since `x >= 0` might have tested false, resulting in the variable `i` being unassigned.</span></span>

#### <a name="-conditional-or-expressions"></a><span data-ttu-id="5d87d-391">|| (podmiňovací operátor OR) výrazů</span><span class="sxs-lookup"><span data-stu-id="5d87d-391">|| (conditional OR) expressions</span></span>

<span data-ttu-id="5d87d-392">Pro výraz *expr* formuláře `expr_first || expr_second`:</span><span class="sxs-lookup"><span data-stu-id="5d87d-392">For an expression *expr* of the form `expr_first || expr_second`:</span></span>

*  <span data-ttu-id="5d87d-393">Stav jednoznačného přiřazení *v* před *expr_first* je stejné jako stav jednoznačného přiřazení *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-393">The definite assignment state of *v* before *expr_first* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="5d87d-394">Stav jednoznačného přiřazení *v* před *expr_second* je jednoznačně přiřazovat, pokud státu *v* po *expr_first* je buď určitě přiřazené nebo "přiřazené jednoznačně po výrazu false".</span><span class="sxs-lookup"><span data-stu-id="5d87d-394">The definite assignment state of *v* before *expr_second* is definitely assigned if the state of *v* after *expr_first* is either definitely assigned or "definitely assigned after false expression".</span></span> <span data-ttu-id="5d87d-395">V opačném případě ji není jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-395">Otherwise, it is not definitely assigned.</span></span>
*  <span data-ttu-id="5d87d-396">Příkaz jednoznačného přiřazení *v* po *expr* se určuje podle:</span><span class="sxs-lookup"><span data-stu-id="5d87d-396">The definite assignment statement of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="5d87d-397">Pokud *expr_first* je konstantní výraz s hodnotou `true`, pak jednoznačného přiřazení stavu *v* po *expr* je stejný jako jednoznačného přiřazení Stav *v* po *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-397">If *expr_first* is a constant expression with the value `true`, then the definite assignment state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_first*.</span></span>
    * <span data-ttu-id="5d87d-398">Jinak, pokud státu *v* po *expr_first* jednoznačně přiřazena, potom stav *v* po *expr* jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-398">Otherwise, if the state of *v* after *expr_first* is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-399">Jinak, pokud stav *v* po *expr_second* jednoznačně přiřazena a stav *v* po *expr_first* je "jednoznačně přiřazené po hodnotu true, výraz"a pak stav *v* po *expr* jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-399">Otherwise, if the state of *v* after *expr_second* is definitely assigned, and the state of *v* after *expr_first* is "definitely assigned after true expression", then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-400">Jinak, pokud státu *v* po *expr_second* jednoznačně přiřazena nebo "přiřazené jednoznačně po false výraz", potom stav *v* po *expr* "jednoznačně přiřazena po výrazu false".</span><span class="sxs-lookup"><span data-stu-id="5d87d-400">Otherwise, if the state of *v* after *expr_second* is definitely assigned or "definitely assigned after false expression", then the state of *v* after *expr* is "definitely assigned after false expression".</span></span>
    * <span data-ttu-id="5d87d-401">Jinak, pokud státu *v* po *expr_first* je "přiřazené jednoznačně po hodnotu true, výraz" a stav *v* po *expr_second*"přiřazené jednoznačně po hodnotu true, výraz", je stav *v* po *expr* "jednoznačně přiřazena po Výraz true".</span><span class="sxs-lookup"><span data-stu-id="5d87d-401">Otherwise, if the state of *v* after *expr_first* is "definitely assigned after true expression", and the state of *v* after *expr_second* is "definitely assigned after true expression", then the state of *v* after *expr* is "definitely assigned after true expression".</span></span>
    * <span data-ttu-id="5d87d-402">V opačném případě stav *v* po *expr* není jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-402">Otherwise, the state of *v* after *expr* is not definitely assigned.</span></span>

<span data-ttu-id="5d87d-403">V příkladu</span><span class="sxs-lookup"><span data-stu-id="5d87d-403">In the example</span></span>
```csharp
class A
{
...
```
<span data-ttu-id="5d87d-404">proměnné `i` se považuje za jednoznačně přiřazené v jednom z vložené příkazy `if` příkazu, ale ne v jiném.</span><span class="sxs-lookup"><span data-stu-id="5d87d-404">the variable `i` is considered definitely assigned in one of the embedded statements of an `if` statement but not in the other.</span></span> <span data-ttu-id="5d87d-405">V `if` příkaz v metodě `G`, proměnná `i` jednoznačně přiřazena v druhém příkazu vložený protože provádění výrazu `(i = y)` vždy předchází spuštění tohoto příkazu embedded.</span><span class="sxs-lookup"><span data-stu-id="5d87d-405">In the `if` statement in method `G`, the variable `i` is definitely assigned in the second embedded statement because execution of the expression `(i = y)` always precedes execution of this embedded statement.</span></span> <span data-ttu-id="5d87d-406">Naproti tomu, proměnná `i` není jednoznačně přiřazena v první příkaz vložený, protože `x >= 0` může otestovali nastavena hodnota true, což vede k proměnné `i` se nepřiřazený.</span><span class="sxs-lookup"><span data-stu-id="5d87d-406">In contrast, the variable `i` is not definitely assigned in the first embedded statement, since `x >= 0` might have tested true, resulting in the variable `i` being unassigned.</span></span>

#### <a name="-logical-negation-expressions"></a><span data-ttu-id="5d87d-407">!</span><span class="sxs-lookup"><span data-stu-id="5d87d-407">!</span></span> <span data-ttu-id="5d87d-408">výrazy (Logická negace)</span><span class="sxs-lookup"><span data-stu-id="5d87d-408">(logical negation) expressions</span></span>

<span data-ttu-id="5d87d-409">Pro výraz *expr* formuláře `! expr_operand`:</span><span class="sxs-lookup"><span data-stu-id="5d87d-409">For an expression *expr* of the form `! expr_operand`:</span></span>

*  <span data-ttu-id="5d87d-410">Stav jednoznačného přiřazení *v* před *expr_operand* je stejné jako stav jednoznačného přiřazení *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-410">The definite assignment state of *v* before *expr_operand* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="5d87d-411">Stav jednoznačného přiřazení *v* po *expr* se určuje podle:</span><span class="sxs-lookup"><span data-stu-id="5d87d-411">The definite assignment state of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="5d87d-412">Pokud stav *v* po \* expr_operand \* je jednoznačně přiřazena, potom stav *v* po *expr* jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-412">If the state of *v* after \*expr_operand \*is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-413">Pokud státu *v* po \* expr_operand \* není jednoznačně přiřazena, potom stav *v* po *expr* není jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-413">If the state of *v* after \*expr_operand \*is not definitely assigned, then the state of *v* after *expr* is not definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-414">Pokud stav *v* po \* expr_operand \* "přiřazené jednoznačně po false výraz", je stav *v* po *expr* "jednoznačně přiřazena po true výraz".</span><span class="sxs-lookup"><span data-stu-id="5d87d-414">If the state of *v* after \*expr_operand \*is "definitely assigned after false expression", then the state of *v* after *expr* is "definitely assigned after true expression".</span></span>
    * <span data-ttu-id="5d87d-415">Pokud stav *v* po \* expr_operand \* "přiřazené jednoznačně po hodnotu true, výraz", je stav *v* po *expr* "jednoznačně přiřazena po false výraz".</span><span class="sxs-lookup"><span data-stu-id="5d87d-415">If the state of *v* after \*expr_operand \*is "definitely assigned after true expression", then the state of *v* after *expr* is "definitely assigned after false expression".</span></span>

#### <a name="-null-coalescing-expressions"></a><span data-ttu-id="5d87d-416">??</span><span class="sxs-lookup"><span data-stu-id="5d87d-416">??</span></span> <span data-ttu-id="5d87d-417">výrazy (nulové sloučení)</span><span class="sxs-lookup"><span data-stu-id="5d87d-417">(null coalescing) expressions</span></span>

<span data-ttu-id="5d87d-418">Pro výraz *expr* formuláře `expr_first ?? expr_second`:</span><span class="sxs-lookup"><span data-stu-id="5d87d-418">For an expression *expr* of the form `expr_first ?? expr_second`:</span></span>

*  <span data-ttu-id="5d87d-419">Stav jednoznačného přiřazení *v* před *expr_first* je stejné jako stav jednoznačného přiřazení *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-419">The definite assignment state of *v* before *expr_first* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="5d87d-420">Stav jednoznačného přiřazení *v* před *expr_second* je stejné jako stav jednoznačného přiřazení *v* po *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-420">The definite assignment state of *v* before *expr_second* is the same as the definite assignment state of *v* after *expr_first*.</span></span>
*  <span data-ttu-id="5d87d-421">Příkaz jednoznačného přiřazení *v* po *expr* se určuje podle:</span><span class="sxs-lookup"><span data-stu-id="5d87d-421">The definite assignment statement of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="5d87d-422">Pokud *expr_first* je konstantní výraz ([konstantní výrazy](expressions.md#constant-expressions)) s hodnotou null, pak stav *v* po *expr* je stejný jako Stav *v* po *expr_second*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-422">If *expr_first* is a constant expression ([Constant expressions](expressions.md#constant-expressions)) with value null, then the state of *v* after *expr* is the same as the state of *v* after *expr_second*.</span></span>
*  <span data-ttu-id="5d87d-423">V opačném případě stav *v* po *expr* je stejné jako stav jednoznačného přiřazení *v* po *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-423">Otherwise, the state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_first*.</span></span>

#### <a name="-conditional-expressions"></a><span data-ttu-id="5d87d-424">?: (podmínky)</span><span class="sxs-lookup"><span data-stu-id="5d87d-424">?: (conditional) expressions</span></span>

<span data-ttu-id="5d87d-425">Pro výraz *expr* formuláře `expr_cond ? expr_true : expr_false`:</span><span class="sxs-lookup"><span data-stu-id="5d87d-425">For an expression *expr* of the form `expr_cond ? expr_true : expr_false`:</span></span>

*  <span data-ttu-id="5d87d-426">Stav jednoznačného přiřazení *v* před *expr_cond* je stejné jako stav *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-426">The definite assignment state of *v* before *expr_cond* is the same as the state of *v* before *expr*.</span></span>
*  <span data-ttu-id="5d87d-427">Stav jednoznačného přiřazení *v* před *expr_true* jednoznačně přiřazena pouze v případě obsahuje jeden z následujících akcí:</span><span class="sxs-lookup"><span data-stu-id="5d87d-427">The definite assignment state of *v* before *expr_true* is definitely assigned if and only if one of the following holds:</span></span>
    * <span data-ttu-id="5d87d-428">*expr_cond* je konstantní výraz s hodnotou `false`</span><span class="sxs-lookup"><span data-stu-id="5d87d-428">*expr_cond* is a constant expression with the value `false`</span></span>
    * <span data-ttu-id="5d87d-429">Stav *v* po *expr_cond* je jednoznačně přiřazovat nebo "jednoznačně přiřazena po Výraz true".</span><span class="sxs-lookup"><span data-stu-id="5d87d-429">the state of *v* after *expr_cond* is definitely assigned or "definitely assigned after true expression".</span></span>
*  <span data-ttu-id="5d87d-430">Stav jednoznačného přiřazení *v* před *expr_false* jednoznačně přiřazena pouze v případě obsahuje jeden z následujících akcí:</span><span class="sxs-lookup"><span data-stu-id="5d87d-430">The definite assignment state of *v* before *expr_false* is definitely assigned if and only if one of the following holds:</span></span>
    * <span data-ttu-id="5d87d-431">*expr_cond* je konstantní výraz s hodnotou `true`</span><span class="sxs-lookup"><span data-stu-id="5d87d-431">*expr_cond* is a constant expression with the value `true`</span></span>
*  <span data-ttu-id="5d87d-432">Stav *v* po *expr_cond* je jednoznačně přiřazovat nebo "jednoznačně přiřazena po výrazu false".</span><span class="sxs-lookup"><span data-stu-id="5d87d-432">the state of *v* after *expr_cond* is definitely assigned or "definitely assigned after false expression".</span></span>
*  <span data-ttu-id="5d87d-433">Stav jednoznačného přiřazení *v* po *expr* se určuje podle:</span><span class="sxs-lookup"><span data-stu-id="5d87d-433">The definite assignment state of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="5d87d-434">Pokud *expr_cond* je konstantní výraz ([konstantní výrazy](expressions.md#constant-expressions)) s hodnotou `true` pak stav *v* po *expr* je stejný jako stav *v* po *expr_true*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-434">If *expr_cond* is a constant expression ([Constant expressions](expressions.md#constant-expressions)) with value `true` then the state of *v* after *expr* is the same as the state of *v* after *expr_true*.</span></span>
    * <span data-ttu-id="5d87d-435">Jinak, pokud *expr_cond* je konstantní výraz ([konstantní výrazy](expressions.md#constant-expressions)) s hodnotou `false` pak stav *v* po *expr* je stejné jako stav *v* po *expr_false*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-435">Otherwise, if *expr_cond* is a constant expression ([Constant expressions](expressions.md#constant-expressions)) with value `false` then the state of *v* after *expr* is the same as the state of *v* after *expr_false*.</span></span>
    * <span data-ttu-id="5d87d-436">Jinak, pokud státu *v* po *expr_true* je jednoznačně přiřazovat a stav *v* po *expr_false* se o výborný prostředek pak přiřazen stav *v* po *expr* jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-436">Otherwise, if the state of *v* after *expr_true* is definitely assigned and the state of *v* after *expr_false* is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="5d87d-437">V opačném případě stav *v* po *expr* není jednoznačně přiřazena.</span><span class="sxs-lookup"><span data-stu-id="5d87d-437">Otherwise, the state of *v* after *expr* is not definitely assigned.</span></span>

#### <a name="anonymous-functions"></a><span data-ttu-id="5d87d-438">Anonymní funkce</span><span class="sxs-lookup"><span data-stu-id="5d87d-438">Anonymous functions</span></span>

<span data-ttu-id="5d87d-439">Pro *lambda_expression* nebo *anonymous_method_expression* *expr* s tělem (buď *bloku* nebo *výraz* ) *tělo*:</span><span class="sxs-lookup"><span data-stu-id="5d87d-439">For a *lambda_expression* or *anonymous_method_expression* *expr* with a body (either *block* or *expression*) *body*:</span></span>

*  <span data-ttu-id="5d87d-440">Stav jednoznačného přiřazení vnější proměnná *v* před *tělo* je stejné jako stav *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-440">The definite assignment state of an outer variable *v* before *body* is the same as the state of *v* before *expr*.</span></span> <span data-ttu-id="5d87d-441">To znamená jednoznačného přiřazení stavu vnější proměnné je zděděno z kontextu anonymní funkce.</span><span class="sxs-lookup"><span data-stu-id="5d87d-441">That is, definite assignment state of outer variables is inherited from the context of the anonymous function.</span></span>
*  <span data-ttu-id="5d87d-442">Stav jednoznačného přiřazení vnější proměnná *v* po *expr* je stejné jako stav *v* před *expr*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-442">The definite assignment state of an outer variable *v* after *expr* is the same as the state of *v* before *expr*.</span></span>

<span data-ttu-id="5d87d-443">V příkladu</span><span class="sxs-lookup"><span data-stu-id="5d87d-443">The example</span></span>
```csharp
...
```
<span data-ttu-id="5d87d-444">vygeneruje chybu kompilace od `max` není jednoznačně přiřazena, ve kterém je deklarována jako anonymní funkce.</span><span class="sxs-lookup"><span data-stu-id="5d87d-444">generates a compile-time error since `max` is not definitely assigned where the anonymous function is declared.</span></span> <span data-ttu-id="5d87d-445">V příkladu</span><span class="sxs-lookup"><span data-stu-id="5d87d-445">The example</span></span>
```csharp
delegate void D();
...
```
<span data-ttu-id="5d87d-446">také vygeneruje chybu kompilace od přiřazení `n` v anonymní funkce nemá žádný vliv na stav jednoznačného přiřazení `n` vně anonymní funkce.</span><span class="sxs-lookup"><span data-stu-id="5d87d-446">also generates a compile-time error since the assignment to `n` in the anonymous function has no affect on the definite assignment state of `n` outside the anonymous function.</span></span>

## <a name="variable-references"></a><span data-ttu-id="5d87d-447">Odkazy na proměnné</span><span class="sxs-lookup"><span data-stu-id="5d87d-447">Variable references</span></span>

<span data-ttu-id="5d87d-448">A *variable_reference* je *výraz* , který je klasifikován jako proměnnou.</span><span class="sxs-lookup"><span data-stu-id="5d87d-448">A *variable_reference* is an *expression* that is classified as a variable.</span></span> <span data-ttu-id="5d87d-449">A *variable_reference* označuje umístění úložiště, který je přístupný načíst aktuální hodnotu a uložte novou hodnotu.</span><span class="sxs-lookup"><span data-stu-id="5d87d-449">A *variable_reference* denotes a storage location that can be accessed both to fetch the current value and to store a new value.</span></span>
...
<span data-ttu-id="5d87d-450">V jazyce C a C++, *variable_reference* se označuje jako *l-hodnoty*.</span><span class="sxs-lookup"><span data-stu-id="5d87d-450">In C and C++, a *variable_reference* is known as an *lvalue*.</span></span>

## <a name="atomicity-of-variable-references"></a><span data-ttu-id="5d87d-451">Atomicitu odkazy na proměnné</span><span class="sxs-lookup"><span data-stu-id="5d87d-451">Atomicity of variable references</span></span>

<span data-ttu-id="5d87d-452">Čtení a zápisy následující datové typy jsou atomické: `bool`, `char`, `byte`, `sbyte`, `short`, `ushort`, `uint`, `int`, `float`a typy odkazů.</span><span class="sxs-lookup"><span data-stu-id="5d87d-452">Reads and writes of the following data types are atomic: `bool`, `char`, `byte`, `sbyte`, `short`, `ushort`, `uint`, `int`, `float`, and reference types.</span></span> <span data-ttu-id="5d87d-453">Kromě toho přečtených a zapsaných typů výčtů se základním typem v předchozím seznamu jsou také atomické.</span><span class="sxs-lookup"><span data-stu-id="5d87d-453">In addition, reads and writes of enum types with an underlying type in the previous list are also atomic.</span></span> <span data-ttu-id="5d87d-454">Operace čtení a zápisu z ostatních typů, včetně `long`, `ulong`, `double`, a `decimal`, a také uživatelem definovaných typů, nemusí být atomické.</span><span class="sxs-lookup"><span data-stu-id="5d87d-454">Reads and writes of other types, including `long`, `ulong`, `double`, and `decimal`, as well as user-defined types, are not guaranteed to be atomic.</span></span> <span data-ttu-id="5d87d-455">Kromě funkcí knihovny určená pro tento účel neexistuje žádná záruka z atomických čtení modify-write, například v případě zvýšení nebo snížení.</span><span class="sxs-lookup"><span data-stu-id="5d87d-455">Aside from the library functions designed for that purpose, there is no guarantee of atomic read-modify-write, such as in the case of increment or decrement.</span></span>
