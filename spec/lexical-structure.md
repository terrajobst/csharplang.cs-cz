...
### <a name="white-space"></a><span data-ttu-id="d5da3-153">Prázdné znaky</span><span class="sxs-lookup"><span data-stu-id="d5da3-153">White space</span></span>
...
### <a name="identifiers"></a><span data-ttu-id="d5da3-171">Identifikátory</span><span class="sxs-lookup"><span data-stu-id="d5da3-171">Identifiers</span></span>
...
<span data-ttu-id="d5da3-202">Typ *boolean_literal* je `bool`.</span><span class="sxs-lookup"><span data-stu-id="d5da3-202">The type of a *boolean_literal* is `bool`.</span></span>
...
#### <a name="character-literals"></a><span data-ttu-id="d5da3-235">Literály znaků</span><span class="sxs-lookup"><span data-stu-id="d5da3-235">Character literals</span></span>
...
<span data-ttu-id="d5da3-298">Příklady TODO</span><span class="sxs-lookup"><span data-stu-id="d5da3-298">Examples TODO</span></span>
...
## <a name="pre-processing-directives"></a><span data-ttu-id="d5da3-308">Direktivy předběžného zpracování</span><span class="sxs-lookup"><span data-stu-id="d5da3-308">Pre-processing directives</span></span>
...
<span data-ttu-id="d5da3-327">Vzhledem k tomu, že tyto dva programy jsou proto v lexikálním rozdílovém, syntakticky stejné.</span><span class="sxs-lookup"><span data-stu-id="d5da3-327">Thus, whereas lexically, the two programs are quite different, syntactically, they are identical.</span></span>
...
<span data-ttu-id="d5da3-347">Direktivy deklarace slouží k definování nebo zrušení definování symbolů podmíněné kompilace.</span><span class="sxs-lookup"><span data-stu-id="d5da3-347">The declaration directives are used to define or undefine conditional compilation symbols.</span></span>
...
<span data-ttu-id="d5da3-348">Zpracování `#define` direktiv způsobí, že daný symbol podmíněné kompilace bude definován, počínaje zdrojovým řádkem, který následuje po direktivě.</span><span class="sxs-lookup"><span data-stu-id="d5da3-348">The processing of a `#define` directive causes the given conditional compilation symbol to become defined, starting with the source line that follows the directive.</span></span> <span data-ttu-id="d5da3-349">Podobně zpracování `#undef` direktiv způsobí, že se daný symbol podmíněné kompilace stane nedefinovaným, počínaje zdrojovým řádkem, který následuje po direktivě.</span><span class="sxs-lookup"><span data-stu-id="d5da3-349">Likewise, the processing of an `#undef` directive causes the given conditional compilation symbol to become undefined, starting with the source line that follows the directive.</span></span>

<span data-ttu-id="d5da3-350">Všechny `#define` direktivy ve zdrojovém souboru musí nastat před prvním tokenem ([tokeny](lexical-structure.md#tokens)) ve zdrojovém souboru. v opačném případě dojde k chybě při kompilaci. `#undef`</span><span class="sxs-lookup"><span data-stu-id="d5da3-350">Any `#define` and `#undef` directives in a source file must occur before the first *token* ([Tokens](lexical-structure.md#tokens)) in the source file; otherwise a compile-time error occurs.</span></span> <span data-ttu-id="d5da3-351">V intuitivních `#define` výrazech `#undef` a direktivy musí předcházet libovolnému "skutečnému kódu" ve zdrojovém souboru.</span><span class="sxs-lookup"><span data-stu-id="d5da3-351">In intuitive terms, `#define` and `#undef` directives must precede any "real code" in the source file.</span></span>

<span data-ttu-id="d5da3-352">Příklad:</span><span class="sxs-lookup"><span data-stu-id="d5da3-352">The example:</span></span>
```csharp
#define Enterprise
...
```
<span data-ttu-id="d5da3-353">je platný, protože `#define` direktivy předcházejí prvnímu tokenu `namespace` (klíčové slovo) ve zdrojovém souboru.</span><span class="sxs-lookup"><span data-stu-id="d5da3-353">is valid because the `#define` directives precede the first token (the `namespace` keyword) in the source file.</span></span>

<span data-ttu-id="d5da3-354">V následujícím příkladu dojde k chybě při kompilaci, protože `#define` následuje skutečný kód:</span><span class="sxs-lookup"><span data-stu-id="d5da3-354">The following example results in a compile-time error because a `#define` follows real code:</span></span>
```csharp
#define A
namespace N
{
...
<span data-ttu-id="d5da3-355">Může definovat symbol podmíněné kompilace, který je již definován, aniž by došlo `#undef` k jeho použití pro tento symbol. `#define`</span><span class="sxs-lookup"><span data-stu-id="d5da3-355">A `#define` may define a conditional compilation symbol that is already defined, without there being any intervening `#undef` for that symbol.</span></span> <span data-ttu-id="d5da3-356">Následující příklad definuje podmíněný kompilační symbol `A` a pak ho znovu definuje.</span><span class="sxs-lookup"><span data-stu-id="d5da3-356">The example below defines a conditional compilation symbol `A` and then defines it again.</span></span>
```csharp
#define A
#define A
```

<span data-ttu-id="d5da3-357">`#undef` Může "undefine" undefine "undefined" – symbol podmíněné kompilace, který není definován.</span><span class="sxs-lookup"><span data-stu-id="d5da3-357">A `#undef` may "undefine" a conditional compilation symbol that is not defined.</span></span> <span data-ttu-id="d5da3-358">Následující příklad definuje podmíněný kompilační symbol `A` a pak je nedefinuje dvakrát; i když druhý `#undef` nemá žádný vliv, je stále platný.</span><span class="sxs-lookup"><span data-stu-id="d5da3-358">The example below defines a conditional compilation symbol `A` and then undefines it twice; although the second `#undef` has no effect, it is still valid.</span></span>
```csharp
#define A
#undef A
#undef A
...
### <a name="conditional-compilation-directives"></a><span data-ttu-id="d5da3-359">Direktivy podmíněné kompilace</span><span class="sxs-lookup"><span data-stu-id="d5da3-359">Conditional compilation directives</span></span>

<span data-ttu-id="d5da3-360">Direktivy podmíněné kompilace jsou používány k podmíněnému zahrnutí nebo vyloučení částí zdrojového souboru.</span><span class="sxs-lookup"><span data-stu-id="d5da3-360">The conditional compilation directives are used to conditionally include or exclude portions of a source file.</span></span>
...
<span data-ttu-id="d5da3-361">Jak je určeno syntaxí, direktivy podmíněné kompilace musí být zapsány jako sady, které se skládají `#if` z, v pořadí, `#elif` direktiva, nula nebo `#else` více direktiv, nula `#endif` nebo jedna direktiva a direktiva.</span><span class="sxs-lookup"><span data-stu-id="d5da3-361">As indicated by the syntax, conditional compilation directives must be written as sets consisting of, in order, an `#if` directive, zero or more `#elif` directives, zero or one `#else` directive, and an `#endif` directive.</span></span> <span data-ttu-id="d5da3-362">Mezi direktivami jsou podmíněné oddíly zdrojového kódu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-362">Between the directives are conditional sections of source code.</span></span> <span data-ttu-id="d5da3-363">Každý oddíl je řízen hned předchozí direktivou.</span><span class="sxs-lookup"><span data-stu-id="d5da3-363">Each section is controlled by the immediately preceding directive.</span></span> <span data-ttu-id="d5da3-364">Podmíněný oddíl může sám obsahovat vnořené direktivy podmíněné kompilace, pokud tyto direktivy tvoří kompletní sady.</span><span class="sxs-lookup"><span data-stu-id="d5da3-364">A conditional section may itself contain nested conditional compilation directives provided these directives form complete sets.</span></span>

<span data-ttu-id="d5da3-365">*Pp_conditional* vybere maximálně jeden z obsažených *conditional_section*s pro normální lexikální zpracování:</span><span class="sxs-lookup"><span data-stu-id="d5da3-365">A *pp_conditional* selects at most one of the contained *conditional_section*s for normal lexical processing:</span></span>

*  <span data-ttu-id="d5da3-366">*Pp_expression*s direktivami `#if` a `#elif` jsou vyhodnocovány v pořadí, dokud jedna z hodnot `true`.</span><span class="sxs-lookup"><span data-stu-id="d5da3-366">The *pp_expression*s of the `#if` and `#elif` directives are evaluated in order until one yields `true`.</span></span> <span data-ttu-id="d5da3-367">Pokud výraz vrací `true`, je vybrána *conditional_section* odpovídající direktiva.</span><span class="sxs-lookup"><span data-stu-id="d5da3-367">If an expression yields `true`, the *conditional_section* of the corresponding directive is selected.</span></span>
*  <span data-ttu-id="d5da3-368">Pokud je *pp_expressiona*`false` a pokud je přítomna direktiva `#else`, vybere se *conditional_section* direktiva `#else`.</span><span class="sxs-lookup"><span data-stu-id="d5da3-368">If all *pp_expression*s yield `false`, and if an `#else` directive is present, the *conditional_section* of the `#else` directive is selected.</span></span>
*  <span data-ttu-id="d5da3-369">V opačném případě není vybrána žádná *conditional_section* .</span><span class="sxs-lookup"><span data-stu-id="d5da3-369">Otherwise, no *conditional_section* is selected.</span></span>

<span data-ttu-id="d5da3-370">Vybraná *conditional_section*, pokud existuje, je zpracována jako normální *input_section*: zdrojový kód obsažený v části musí splňovat lexikální gramatiku; tokeny jsou generovány ze zdrojového kódu v oddílu; a direktivy předběžného zpracování v oddílu mají předepsané účinky.</span><span class="sxs-lookup"><span data-stu-id="d5da3-370">The selected *conditional_section*, if any, is processed as a normal *input_section*: the source code contained in the section must adhere to the lexical grammar; tokens are generated from the source code in the section; and pre-processing directives in the section have the prescribed effects.</span></span>

<span data-ttu-id="d5da3-371">Zbývající *conditional_section*, pokud nějaké jsou zpracovávány jako *skipped_section*s s výjimkou předzpracování direktiv, zdrojový kód v části nemusí vyhovovat lexikální gramatice; v oddílu nejsou vygenerovány žádné tokeny ze zdrojového kódu. a direktivy předběžného zpracování v oddílu musí být lexikální, ale nejsou jinak zpracovány.</span><span class="sxs-lookup"><span data-stu-id="d5da3-371">The remaining *conditional_section*s, if any, are processed as *skipped_section*s: except for pre-processing directives, the source code in the section need not adhere to the lexical grammar; no tokens are generated from the source code in the section; and pre-processing directives in the section must be lexically correct but are not otherwise processed.</span></span> <span data-ttu-id="d5da3-372">V rámci *conditional_section* , který se zpracovává jako *skipped_section*, všechny vnořené *conditional_section*s (obsažené ve vnořených `#if`... `#endif` a `#region`... `#endregion`) se také zpracovávají jako skipped_.  *oddíl*s.</span><span class="sxs-lookup"><span data-stu-id="d5da3-372">Within a *conditional_section* that is being processed as a *skipped_section*, any nested *conditional_section*s (contained in nested `#if`...`#endif` and `#region`...`#endregion` constructs) are also processed as *skipped_section*s.</span></span>

<span data-ttu-id="d5da3-373">Následující příklad ukazuje, jak mohou direktivy podmíněné kompilace vnořovat:</span><span class="sxs-lookup"><span data-stu-id="d5da3-373">The following example illustrates how conditional compilation directives can nest:</span></span>
```csharp
...
<span data-ttu-id="d5da3-374">S výjimkou předběžně zpracovávaných direktiv nepodléhá zdrojový kód lexikální analýze.</span><span class="sxs-lookup"><span data-stu-id="d5da3-374">Except for pre-processing directives, skipped source code is not subject to lexical analysis.</span></span> <span data-ttu-id="d5da3-375">Následující příklad je platný bez ohledu na neukončený komentář v `#else` části:</span><span class="sxs-lookup"><span data-stu-id="d5da3-375">For example, the following is valid despite the unterminated comment in the `#else` section:</span></span>
```csharp
#define Debug        // Debugging on
...
<span data-ttu-id="d5da3-376">Všimněte si však, že direktivy předběžného zpracování jsou nutné k lexikálnímu opravení i v vynechaných částech zdrojového kódu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-376">Note, however, that pre-processing directives are required to be lexically correct even in skipped sections of source code.</span></span>

<span data-ttu-id="d5da3-377">Direktivy předběžného zpracování nejsou zpracovávány při jejich zobrazení uvnitř víceřádkových vstupních prvků.</span><span class="sxs-lookup"><span data-stu-id="d5da3-377">Pre-processing directives are not processed when they appear inside multi-line input elements.</span></span> <span data-ttu-id="d5da3-378">Například program:</span><span class="sxs-lookup"><span data-stu-id="d5da3-378">For example, the program:</span></span>
```csharp
class Hello
{
    static void Main() {
...
```
<span data-ttu-id="d5da3-379">výsledek výstupu:</span><span class="sxs-lookup"><span data-stu-id="d5da3-379">results in the output:</span></span>
```console
hello,
#if Debug
        world
...
<span data-ttu-id="d5da3-380">V běžných případech může sada předzpracovaných direktiv, které jsou zpracovány, záviset na vyhodnocení *pp_expression*.</span><span class="sxs-lookup"><span data-stu-id="d5da3-380">In peculiar cases, the set of pre-processing directives that is processed might depend on the evaluation of the *pp_expression*.</span></span> <span data-ttu-id="d5da3-381">Příklad:</span><span class="sxs-lookup"><span data-stu-id="d5da3-381">The example:</span></span>
```csharp
#if X
    /*
#else
...
#endif
```
<span data-ttu-id="d5da3-382">vždy vytvoří stejný datový proud tokenu`class` ( `}` `Q` `{` ) bez ohledu na to, zda `X` je definován.</span><span class="sxs-lookup"><span data-stu-id="d5da3-382">always produces the same token stream (`class` `Q` `{` `}`), regardless of whether or not `X` is defined.</span></span> <span data-ttu-id="d5da3-383">Pokud `X` je definován, jediné zpracované direktivy jsou `#if` a `#endif`z důvodu víceřádkového komentáře.</span><span class="sxs-lookup"><span data-stu-id="d5da3-383">If `X` is defined, the only processed directives are `#if` and `#endif`, due to the multi-line comment.</span></span> <span data-ttu-id="d5da3-384">Pokud `X` není definován, pak tři direktivy (`#if`, `#else`, `#endif`) jsou součástí sady direktiv.</span><span class="sxs-lookup"><span data-stu-id="d5da3-384">If `X` is undefined, then three directives (`#if`, `#else`, `#endif`) are part of the directive set.</span></span>

### <a name="diagnostic-directives"></a><span data-ttu-id="d5da3-385">Diagnostické direktivy</span><span class="sxs-lookup"><span data-stu-id="d5da3-385">Diagnostic directives</span></span>

<span data-ttu-id="d5da3-386">Diagnostické direktivy slouží k explicitnímu generování chybových zpráv a upozornění, které jsou hlášeny stejným způsobem jako jiné chyby při kompilaci a upozornění.</span><span class="sxs-lookup"><span data-stu-id="d5da3-386">The diagnostic directives are used to explicitly generate error and warning messages that are reported in the same way as other compile-time errors and warnings.</span></span>
...
<span data-ttu-id="d5da3-387">Příklad:</span><span class="sxs-lookup"><span data-stu-id="d5da3-387">The example:</span></span>
```csharp
...
class Test {...}
```
<span data-ttu-id="d5da3-388">vždy vytvoří upozornění ("revize kódu nutná před vrácením se změnami") a vytvoří chybu při kompilaci ("sestavení nemůže být ladění a prodej"), pokud jsou definovány podmíněné symboly `Debug` a. `Retail`</span><span class="sxs-lookup"><span data-stu-id="d5da3-388">always produces a warning ("Code review needed before check-in"), and produces a compile-time error ("A build can't be both debug and retail") if the conditional symbols `Debug` and `Retail` are both defined.</span></span> <span data-ttu-id="d5da3-389">Všimněte si, že *pp_message* může obsahovat libovolný text; konkrétně nemusí obsahovat tokeny ve správném formátu, jak je znázorněno v jedné uvozovkě ve slově `can't`.</span><span class="sxs-lookup"><span data-stu-id="d5da3-389">Note that a *pp_message* can contain arbitrary text; specifically, it need not contain well-formed tokens, as shown by the single quote in the word `can't`.</span></span>

### <a name="region-directives"></a><span data-ttu-id="d5da3-390">Direktivy oblasti</span><span class="sxs-lookup"><span data-stu-id="d5da3-390">Region directives</span></span>

<span data-ttu-id="d5da3-391">Direktivy oblasti se používají k explicitnímu označení oblastí zdrojového kódu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-391">The region directives are used to explicitly mark regions of source code.</span></span>
...
<span data-ttu-id="d5da3-392">K oblasti není připojen žádný sémantický význam. oblasti jsou určeny pro účely programátora nebo automatizovaných nástrojů k označení oddílu zdrojového kódu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-392">No semantic meaning is attached to a region; regions are intended for use by the programmer or by automated tools to mark a section of source code.</span></span> <span data-ttu-id="d5da3-393">Zpráva zadaná v `#region` direktivě nebo `#endregion` má také žádný sémantický význam; slouží pouze k identifikaci oblasti.</span><span class="sxs-lookup"><span data-stu-id="d5da3-393">The message specified in a `#region` or `#endregion` directive likewise has no semantic meaning; it merely serves to identify the region.</span></span> <span data-ttu-id="d5da3-394">Shoda s `#region` a direktivami `#endregion` mohou mít různé *pp_message*.</span><span class="sxs-lookup"><span data-stu-id="d5da3-394">Matching `#region` and `#endregion` directives may have different *pp_message*s.</span></span>

<span data-ttu-id="d5da3-395">Lexikální zpracování oblasti:</span><span class="sxs-lookup"><span data-stu-id="d5da3-395">The lexical processing of a region:</span></span>
```csharp
#region
...
#endregion
```
<span data-ttu-id="d5da3-396">odpovídá přesně lexikálnímu zpracování direktivy podmíněné kompilace ve formátu:</span><span class="sxs-lookup"><span data-stu-id="d5da3-396">corresponds exactly to the lexical processing of a conditional compilation directive of the form:</span></span>
```csharp
#if true
...
#endif
...
### <a name="line-directives"></a><span data-ttu-id="d5da3-397">Line – direktivy</span><span class="sxs-lookup"><span data-stu-id="d5da3-397">Line directives</span></span>

<span data-ttu-id="d5da3-398">Direktivy řádku lze použít k změně čísel řádků a názvů zdrojových souborů, které jsou hlášeny kompilátorem, jako jsou upozornění a chyby a které jsou používány atributy informací o volajícím ([atributy informací o volajícím](attributes.md#caller-info-attributes)).</span><span class="sxs-lookup"><span data-stu-id="d5da3-398">Line directives may be used to alter the line numbers and source file names that are reported by the compiler in output such as warnings and errors, and that are used by caller info attributes ([Caller info attributes](attributes.md#caller-info-attributes)).</span></span>

<span data-ttu-id="d5da3-399">Direktivy řádku se nejčastěji používají v nástrojích pro meta programování, C# které generují zdrojový kód z nějakého jiného textového vstupu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-399">Line directives are most commonly used in meta-programming tools that generate C# source code from some other text input.</span></span>
...
<span data-ttu-id="d5da3-400">Pokud nejsou `#line` přítomny žádné direktivy, kompilátor ohlásí hodnoty true a názvy zdrojových souborů ve svém výstupu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-400">When no `#line` directives are present, the compiler reports true line numbers and source file names in its output.</span></span> <span data-ttu-id="d5da3-401">Při zpracování direktivy `#line`, která obsahuje *line_indicator* , která není `default`, kompilátor zpracovává řádek za direktivou, která má dané číslo řádku (a název souboru, pokud je zadán).</span><span class="sxs-lookup"><span data-stu-id="d5da3-401">When processing a `#line` directive that includes a *line_indicator* that is not `default`, the compiler treats the line after the directive as having the given line number (and file name, if specified).</span></span>

<span data-ttu-id="d5da3-402">`#line default` Direktiva obrátí účinek všech předchozích direktiv #line.</span><span class="sxs-lookup"><span data-stu-id="d5da3-402">A `#line default` directive reverses the effect of all preceding #line directives.</span></span> <span data-ttu-id="d5da3-403">Kompilátor ohlásí hodnotu true pro následné řádky, přesně tak, že nebyly zpracovány `#line` žádné direktivy.</span><span class="sxs-lookup"><span data-stu-id="d5da3-403">The compiler reports true line information for subsequent lines, precisely as if no `#line` directives had been processed.</span></span>

<span data-ttu-id="d5da3-404">`#line hidden` Direktiva nemá žádný vliv na soubor a čísla řádků uvedená v chybových zprávách, ale má vliv na ladění na úrovni zdroje.</span><span class="sxs-lookup"><span data-stu-id="d5da3-404">A `#line hidden` directive has no effect on the file and line numbers reported in error messages, but does affect source level debugging.</span></span> <span data-ttu-id="d5da3-405">Při ladění nejsou všechny řádky mezi `#line hidden` direktivou a následnou `#line` direktivou (které nejsou `#line hidden`) žádné informace o čísle řádku.</span><span class="sxs-lookup"><span data-stu-id="d5da3-405">When debugging, all lines between a `#line hidden` directive and the subsequent `#line` directive (that is not `#line hidden`) have no line number information.</span></span> <span data-ttu-id="d5da3-406">Při procházení kódu v ladicím programu budou tyto řádky zcela vynechány.</span><span class="sxs-lookup"><span data-stu-id="d5da3-406">When stepping through code in the debugger, these lines will be skipped entirely.</span></span>

<span data-ttu-id="d5da3-407">Všimněte si, že se *název_souboru* liší od regulárního řetězcového literálu v tom, že řídicí znaky nejsou zpracovány; znak "`\`" jednoduše označuje normální znak zpětného lomítka v rámci *název_souboru*.</span><span class="sxs-lookup"><span data-stu-id="d5da3-407">Note that a *file_name* differs from a regular string literal in that escape characters are not processed; the "`\`" character simply designates an ordinary backslash character within a *file_name*.</span></span>

### <a name="pragma-directives"></a><span data-ttu-id="d5da3-408">Direktivy pragma</span><span class="sxs-lookup"><span data-stu-id="d5da3-408">Pragma directives</span></span>

<span data-ttu-id="d5da3-409">`#pragma` Direktiva předzpracování se používá k určení volitelných kontextových informací kompilátoru.</span><span class="sxs-lookup"><span data-stu-id="d5da3-409">The `#pragma` preprocessing directive is used to specify optional contextual information to the compiler.</span></span> <span data-ttu-id="d5da3-410">Informace, které jsou zadány v `#pragma` direktivě, nebudou nikdy měnit sémantiku programu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-410">The information supplied in a `#pragma` directive will never change program semantics.</span></span>
...
<span data-ttu-id="d5da3-411">C#poskytuje `#pragma` direktivy pro řízení upozornění kompilátoru.</span><span class="sxs-lookup"><span data-stu-id="d5da3-411">C# provides `#pragma` directives to control compiler warnings.</span></span> <span data-ttu-id="d5da3-412">Budoucí verze jazyka mohou zahrnovat další `#pragma` direktivy.</span><span class="sxs-lookup"><span data-stu-id="d5da3-412">Future versions of the language may include additional `#pragma` directives.</span></span> <span data-ttu-id="d5da3-413">Aby byla zajištěna interoperabilita s jinými C# kompilátory C# , kompilátor společnosti Microsoft nevydá chyby kompilace `#pragma` pro neznámé direktivy; takové direktivy však generují upozornění.</span><span class="sxs-lookup"><span data-stu-id="d5da3-413">To ensure interoperability with other C# compilers, the Microsoft C# compiler does not issue compilation errors for unknown `#pragma` directives; such directives do however generate warnings.</span></span>

#### <a name="pragma-warning"></a><span data-ttu-id="d5da3-414">pragma – upozornění</span><span class="sxs-lookup"><span data-stu-id="d5da3-414">Pragma warning</span></span>

<span data-ttu-id="d5da3-415">`#pragma warning` Direktiva slouží k zakázání nebo obnovení všech nebo konkrétní sady varovných zpráv během kompilace následného textu programu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-415">The `#pragma warning` directive is used to disable or restore all or a particular set of warning messages during compilation of the subsequent program text.</span></span>
...
<span data-ttu-id="d5da3-416">`#pragma warning` Direktiva, která vynechává seznam upozornění, má vliv na všechna upozornění.</span><span class="sxs-lookup"><span data-stu-id="d5da3-416">A `#pragma warning` directive that omits the warning list affects all warnings.</span></span> <span data-ttu-id="d5da3-417">`#pragma warning` Direktiva, která obsahuje seznam upozornění, má vliv pouze na upozornění, která jsou uvedena v seznamu.</span><span class="sxs-lookup"><span data-stu-id="d5da3-417">A `#pragma warning` directive that includes a warning list affects only those warnings that are specified in the list.</span></span>

<span data-ttu-id="d5da3-418">`#pragma warning disable` Direktiva zakáže všechny nebo danou sadu upozornění.</span><span class="sxs-lookup"><span data-stu-id="d5da3-418">A `#pragma warning disable` directive disables all or the given set of warnings.</span></span>

<span data-ttu-id="d5da3-419">`#pragma warning restore` Direktiva obnoví všechny nebo danou sadu upozornění na stav, který byl platný na začátku kompilační jednotky.</span><span class="sxs-lookup"><span data-stu-id="d5da3-419">A `#pragma warning restore` directive restores all or the given set of warnings to the state that was in effect at the beginning of the compilation unit.</span></span> <span data-ttu-id="d5da3-420">Všimněte si, že pokud bylo určité upozornění zakázáno externě, `#pragma warning restore` a (zda pro všechna nebo konkrétní upozornění) nebude toto upozornění znovu povoleno.</span><span class="sxs-lookup"><span data-stu-id="d5da3-420">Note that if a particular warning was disabled externally, a `#pragma warning restore` (whether for all or the specific warning) will not re-enable that warning.</span></span>

<span data-ttu-id="d5da3-421">Následující příklad ukazuje použití `#pragma warning` k dočasnému zakázání upozornění nahlášeného při odkazování na zastaralé členy pomocí čísla upozornění z kompilátoru společnosti Microsoft. C#</span><span class="sxs-lookup"><span data-stu-id="d5da3-421">The following example shows use of `#pragma warning` to temporarily disable the warning reported when obsoleted members are referenced, using the warning number from the Microsoft C# compiler.</span></span>
```csharp
using System;
